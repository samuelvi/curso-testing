...
start_selenium() {
  if [ $START -eq 1 ]; then
      ...
      if [ $? -ne 1 ]; then
          ...
          LANG=en-US java -Dwebdriver.chrome.driver=/usr/local/bin/chromedriver -Dwebdriver.chrome.whitelistedIps="127.0.0.1" -jar /var/www/html/bin/selenium-server-standalone.jar &
    fi
  fi
}
