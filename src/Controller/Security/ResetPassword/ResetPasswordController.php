...
use App\Context\Security\Form\ChangePasswordFormType;
...
class ResetPasswordController extends AbstractController
{
    ...
    public function reset(Request $request, UserPasswordHasherInterface $userPasswordHasherInterface, string $token = null): Response
    {
        ...
        try {
            $userEntity = $this->resetPasswordHelper->validateTokenAndFetchUser($token);
    ...
    {
        $userEntity = $this->getDoctrine()->getRepository(UserEntity::class)->findOneBy([
            'email' => $emailFormData,
        ]);
        ...
        try {
            $resetToken = $this->resetPasswordHelper->generateResetToken($userEntity);
        } catch (ResetPasswordExceptionInterface $e) {
